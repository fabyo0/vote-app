<div
    ...
    <button
        ...
        "
    >
        <svg style="color: #338AF1;" class="h-8 w-8 text-gray-400" viewBox="0 0 20 20"
             fill="currentColor">
            <path
            ...
            @foreach($notifications as $notification)
                <li>
                    ...
                    @elseif($notification->type === 'App\Notifications\NewFollower')
                        <a
                                href="{{ route('user.show', '@' . ($notification->data['follower_username'] ?? $notification->data['follower_id'])) }}"
                                @click.prevent="
                                isOpen = false
                            "
                                wire:click.prevent="markAsRead('{{ $notification->id }}')"
                                class="flex hover:bg-gray-100 transition duration-150 ease-in px-5 py-3"
                        >
                            <img src="{{ $notification->data['follower_avatar'] }}"
                                 class="rounded-xl w-10 h-10" alt="avatar">
                            <div class="ml-4">
                                <div class="line-clamp-6">
                                    <span class="font-semibold">{{ $notification->data['follower_name'] }}</span> started following you
                                </div>
                                <div
                                        class="text-xs text-gray-500 mt-2">{{ $notification->created_at->diffForHumans() }}</div>
                            </div>

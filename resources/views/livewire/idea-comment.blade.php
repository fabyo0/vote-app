<div
    ...
    class="@if ($comment->is_status_update) is-status-update {{ 'status-'.Str::kebab($comment->status->name)}} @endif @if ($comment->isReply()) nested-comment @endif comment-container relative @if ($comment->isReply()) bg-gray-50 @else bg-white @endif rounded-xl flex transition duration-500 ease-in @if ($comment->isReply()) mt-2 @else mt-4 @endif"
>
    <div class="flex flex-col md:flex-row flex-1 @if ($comment->isReply()) px-3 py-3 @else px-4 py-6 @endif">
        ...
        <div class="w-full @if ($comment->isReply()) md:mx-2 @else md:mx-4 @endif">
            <div class="@if ($comment->isReply()) text-sm text-gray-700 @else text-gray-600 @endif">
                @admin
                    @if ($comment->spam_reports > 0)
                        <div class="text-red mb-2 @if ($comment->isReply()) text-xs @endif">Spam Reports: {{ $comment->spam_reports }}</div>
                    @endif
                @endadmin

                @if ($comment->is_status_update)
                    <h4 class="@if ($comment->isReply()) text-lg @else text-xl @endif font-semibold mb-3">
                        Status Changed to <span class="font-bold">"{{ $comment->status->name }}"</span>
                    </h4>
                @endif

                <div class="@if ($comment->isReply()) mt-2 md:mt-0 @else mt-4 md:mt-0 @endif">
                    {{ $comment->body }}
                </div>
            </div>
                ...
                @auth
                    ...
                    >
                        <div class="relative">
                            ...
                            >
                                <!-- Reply Option - Her zaman göster (herhangi bir comment'e reply yazılabilir) -->
                                <li>
                                    <a
                                        href="#"
            ...
            <!-- Reply Form -->
            @if ($showReplyForm)
                <div class="@if ($comment->isReply()) mt-3 ml-3 @else mt-4 ml-4 @endif border-l-2 border-gray-300 pl-4">
                    <livewire:add-reply
            ...
            <!-- Display Replies (Recursive - Unlimited Depth) -->
            @if ($comment->nestedReplies->isNotEmpty())
                <div class="@if ($comment->isReply()) mt-3 ml-3 @else mt-6 ml-4 @endif border-l-2 border-gray-300 pl-4 space-y-2">
